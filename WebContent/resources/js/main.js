...
$(".necesito-btn").click(function(){
		...
		$(".early-necesito").after("<input type='text' class='form-control necesito last-necesito' placeholder='Nombre del Necesito'><label for='categoria-necesito' class='white desc-last-necesito'>¿En qué categoría lo pondrías?</label><select id='categoria-necesito' class='form-control desc-last-necesito'><option value='conocimiento'>Es Conocimiento</option><option value='objeto'>Es un Objeto</option><option value='espacio'>Es un Espacio</option></select><p class='white margined-top desc-last-necesito'><strong>Descripción del Necesito </strong><small class='white'>(Máximo 140 caracteres)</small></p><textarea class='form-control gap desc-last-necesito' rows='2'></textarea>").removeClass("early-link");
...
$("#notif-xs").click(function(){
	$("#body-notif-xs").fadeToggle("0", "linear");
});
