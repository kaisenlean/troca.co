...
public class ProyectoUsuarioDaoImpl extends
		GenericJpaRepository<ProyectoUsuario, Integer> implements
		co.icesi.troca.repositories.proyecto.ProyectoUsuarioDao, Serializable {
	...
	public List<Proyecto> findByUsuario(Usuario usuario) {
		Query query = getEntityManager()
				.createQuery(
						"select p.proyecto from ProyectoUsuario p where p.usuario = :usuario");
	 ...
	 * (non-Javadoc)
	 * 
	 * @see co.icesi.troca.repositories.proyecto.ProyectoUsuarioDao#getByUsuarioAndProyecto(co.icesi.troca.model.usuario.Usuario,
	 *      co.icesi.troca.model.proyecto.Proyecto)
	 */
	@Override
	public ProyectoUsuario getByUsuarioAndProyecto(Usuario usuario,
			Proyecto proyecto) {
		Criterion ct1 = Restrictions.conjunction()
