...
public class BeanNotificacion extends BaseBean implements Serializable {
	...
	public void init() {
		if (login.getUsuario() != null) {
			if (login.getUsuario().getId() != null) {
				...
				lista = notificacionService.findNotificacionesUsuario(login
						.getUsuario());
			} else {
	...
	public String mensajeComplementario(Notificacion notificacion) {
		...
		if (notificacion.getModulo().equals(Modulo.TRUEQUE_NECESITO)
				| notificacion.getModulo().equals(Modulo.TRUEQUE_TENGO)) {
			try {
				Trueque trueque = truequeService.findById(Integer
						.valueOf(notificacion.getReferenceId()));
				if (trueque == null) {
					return "";
				}
